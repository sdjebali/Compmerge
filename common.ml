...
(*******************************************************************************)
(* This file contains small and generic functions that can be used everywhere  *)
(*******************************************************************************)
...
(* this function was added on 03/07/2012 in order to sort in a stranded way or not a gtf file 
   it was added to be able to still have the same input and output for make_sorted_temp_file_if_user_wants
   that is also used in overlap *)
let make_sorted_temp_file_if_user_wants_stranded stranded sorted file =
  let tmpfile=make_temp_file_name file in
    if (not sorted) then
      begin
	if (stranded) then
	  Unix.system (("sort -k7,7 -k1,1 -k4,4n -k5,5n ")^(file)^(" > "^(tmpfile)))
	else
...
(* to have the old function used in overlap *)
let make_sorted_temp_file_if_user_wants = make_sorted_temp_file_if_user_wants_stranded false;;

(* this function was also added in march2012 in order to be able to sort according to strand and tr id
   note that this requires that the tr id is in field no 12. not used in fact *)
let make_sorted_temp_file_if_user_wants_stranded_and_tr stranded sorted file =
  let tmpfile=make_temp_file_name file in
    if (not sorted) then
      begin
	if (stranded) then
	  Unix.system (("sort -k7,7 -k12,12 ")^(file)^(" > "^(tmpfile)))
	else
	  Unix.system (("sort -k12,12 ")^(file)^(" > "^(tmpfile)));
	tmpfile;
      end
    else
      file;;
...
(*******************************************)
(* general overlap, only looking at coordinates (no chr) *)
let foverlap i1 i2 =
...
(**********************************************)
(* Functions on numbers (int and float)       *)
(**********************************************)

let inv_comp a b = -(Pervasives.compare a b);;

let avg_int l = float(List.fold_left ( + ) 0 l) /. float(List.length l);;

let avg_float l = (List.fold_left ( +. ) 0.0 l) /. float(List.length l);;
